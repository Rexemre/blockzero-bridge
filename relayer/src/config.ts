...
export const config = {
  ...
  bloz: {
    ...
    /** Dedicated bz1 address that receives the 3.9% bridge fee on wrap (after mint) and unwrap. */
    feeBz1Address: (() => {
      const v = process.env.BRIDGE_FEE_BZ1_ADDRESS?.trim();
      if (!v) return null;
      if (!v.startsWith("bz1")) {
        throw new Error("BRIDGE_FEE_BZ1_ADDRESS must be a mainnet bz1 address");
      }
      return v;
    })(),
